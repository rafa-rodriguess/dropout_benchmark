...
"""
Production comparable-window Weibull AFT benchmark module for stage D5.11.

What this file does:
- prepares the comparable-window AFT treatment directly from DuckDB-ready enrollment tables
- tunes and fits the official Weibull AFT model under the benchmark protocol
...
- materialize the full comparable AFT benchmark arm deterministically from DuckDB-ready tables without notebook runtime state
...
Expected outputs:
- DuckDB tables table_weibull_aft_preprocessing_summary, table_weibull_aft_raw_feature_manifest,
  table_weibull_aft_feature_names_out, table_weibull_aft_tuning_results,
  table_weibull_aft_tuned_test_predictions, table_weibull_aft_tuned_primary_metrics,
  table_weibull_aft_tuned_brier_by_horizon, table_weibull_aft_tuned_secondary_metrics,
  table_weibull_aft_tuned_td_auc_support_audit, table_weibull_aft_tuned_row_diagnostics,
  table_weibull_aft_tuned_support_by_horizon, table_weibull_aft_tuned_calibration_summary,
  table_weibull_aft_tuned_calibration_bins_by_horizon,
  table_weibull_aft_tuned_predicted_vs_observed_survival
- outputs_benchmark_survival/metadata/weibull_aft_preprocessing_config.json
- outputs_benchmark_survival/metadata/weibull_aft_tuned_model_config.json
- outputs_benchmark_survival/models/weibull_aft_tuned.joblib
- outputs_benchmark_survival/models/weibull_aft_preprocessor.joblib
...
- table_weibull_aft_preprocessing_summary
- table_weibull_aft_raw_feature_manifest
- table_weibull_aft_feature_names_out
- table_weibull_aft_tuning_results
- table_weibull_aft_tuned_test_predictions
- table_weibull_aft_tuned_primary_metrics
- table_weibull_aft_tuned_brier_by_horizon
- table_weibull_aft_tuned_secondary_metrics
- table_weibull_aft_tuned_td_auc_support_audit
- table_weibull_aft_tuned_row_diagnostics
- table_weibull_aft_tuned_support_by_horizon
- table_weibull_aft_tuned_calibration_summary
- table_weibull_aft_tuned_calibration_bins_by_horizon
- table_weibull_aft_tuned_predicted_vs_observed_survival
- pipeline_table_catalog
...
NOTEBOOK_NAME = "dropout_bench_D_v5_11_weibull_aft.ipynb"
PREVIEW_ROWS = 20
...
AFT_DURATION_FLOOR = 1.0
MODEL_NAME = "weibull_aft_tuned"
MODEL_IMPLEMENTATION = "lifelines_weibull_aft"
...
AFT_TUNING_GRID = (
	{"candidate_id": 1,  "penalizer": 0.0001, "l1_ratio": 0.0},
	{"candidate_id": 2,  "penalizer": 0.0002, "l1_ratio": 0.0},
	{"candidate_id": 3,  "penalizer": 0.0005, "l1_ratio": 0.0},
	{"candidate_id": 4,  "penalizer": 0.001,  "l1_ratio": 0.0},
	{"candidate_id": 5,  "penalizer": 0.002,  "l1_ratio": 0.0},
	{"candidate_id": 6,  "penalizer": 0.005,  "l1_ratio": 0.0},
	{"candidate_id": 7,  "penalizer": 0.01,   "l1_ratio": 0.0},
	{"candidate_id": 8,  "penalizer": 0.02,   "l1_ratio": 0.0},
	{"candidate_id": 9,  "penalizer": 0.05,   "l1_ratio": 0.0},
	{"candidate_id": 10, "penalizer": 0.10,   "l1_ratio": 0.0},
	{"candidate_id": 11, "penalizer": 0.15,   "l1_ratio": 0.0},
	{"candidate_id": 12, "penalizer": 0.20,   "l1_ratio": 0.0},
	{"candidate_id": 13, "penalizer": 0.30,   "l1_ratio": 0.0},
	{"candidate_id": 14, "penalizer": 0.40,   "l1_ratio": 0.0},
	{"candidate_id": 15, "penalizer": 0.50,   "l1_ratio": 0.0},
	{"candidate_id": 16, "penalizer": 0.60,   "l1_ratio": 0.0},
	{"candidate_id": 17, "penalizer": 0.70,   "l1_ratio": 0.0},
	{"candidate_id": 18, "penalizer": 0.80,   "l1_ratio": 0.0},
	{"candidate_id": 19, "penalizer": 1.0,    "l1_ratio": 0.0},
	{"candidate_id": 20, "penalizer": 1.5,    "l1_ratio": 0.0},
	{"candidate_id": 21, "penalizer": 2.0,    "l1_ratio": 0.0},
	{"candidate_id": 22, "penalizer": 3.0,    "l1_ratio": 0.0},
	{"candidate_id": 23, "penalizer": 4.0,    "l1_ratio": 0.0},
	{"candidate_id": 24, "penalizer": 5.0,    "l1_ratio": 0.0},
)
...
class LowVarianceFilter(BaseEstimator, TransformerMixin):
	...
	def fit(self, X: Any, y: Any = None) -> "LowVarianceFilter":
		...
		if not bool(np.any(self.support_mask_)):
			raise ValueError("All transformed Weibull AFT features were removed by the low-variance filter.")
		return self
...
def materialize_dataframe_table(
	...
	temp_view_name = "__stage_d_5_11_materialize_df__"
	try:
...
def initialize_context(window_weeks: int) -> PipelineContext:
	...
	# - loads benchmark configuration, run metadata, and opens the deterministic DuckDB runtime.
	# Inputs:
	...
	# - DuckDB tables read: none directly; pipeline catalog bootstrap may inspect or initialize internal registry tables.
	# - configuration values used: benchmark seed, test size, early window weeks, main enrollment window weeks, and shared path settings.
	# Outputs:
	# - files generated: none.
	# - DuckDB tables created or updated: pipeline_table_catalog and vw_pipeline_table_catalog_schema may be initialized or refreshed by util helpers.
	# - objects returned in memory: PipelineContext with open DuckDB connection and resolved paths.
	...
	log_stage_start("5.11.1", "Lightweight runtime bootstrap")
	...
	require_tables(con, [input_train_table, input_test_table], block_number="5.11.1")
	...
	log_stage_end("5.11.1")
	return ctx


def build_aft_treatment(ctx: PipelineContext) -> AFTTreatmentState:
	# What this block does:
	# - builds the deterministic comparable-arm preprocessing pipeline and transforms train/test enrollment tables.
	# Inputs:
	# - files read: benchmark_modeling_contract.toml through the already loaded context.
	# - DuckDB tables read: enrollment_cox_ready_train and enrollment_cox_ready_test.
	# - configuration values used: feature_contract static, main enrollment window, and optional comparable features.
	# Outputs:
	# - files generated: outputs_benchmark_survival/metadata/weibull_aft_preprocessing_config.json.
	# - DuckDB tables created or updated: table_weibull_aft_preprocessing_summary, table_weibull_aft_raw_feature_manifest, table_weibull_aft_feature_names_out.
	# - objects returned in memory: AFTTreatmentState with transformed matrices, fitted preprocessing pipeline, and feature manifests.
	log_stage_start("5.11.2", "Build comparable-window Weibull AFT treatment")
	...
	if sorted(expected_comparable_numeric_features) != sorted(numeric_features):
		raise ValueError(
			"The comparable Weibull AFT numeric feature specification does not match the canonical modeling contract. "
	...
	train_df = load_required_table(ctx, ctx.input_train_table, required_columns, block_number="5.11.2")
	test_df = load_required_table(ctx, ctx.input_test_table, required_columns, block_number="5.11.2")
	...
	if X_train.ndim != 2 or X_test.ndim != 2:
		raise ValueError(f"The comparable Weibull AFT transformed matrices must be 2D. Got train={X_train.shape}, test={X_test.shape}.")
	if not np.isfinite(X_train).all() or not np.isfinite(X_test).all():
		raise ValueError("The comparable Weibull AFT transformed matrices contain non-finite values.")
		...
		[
			{
				...
				"aft_positioning_note": "early-window comparable Weibull AFT benchmark",
	...
	materialize_dataframe_table(
		...
		table_name=artifact_name(ctx, "table_weibull_aft_preprocessing_summary"),
		block_number="5.11.2",
		label="Stage 5.11.2 table_weibull_aft_preprocessing_summary - Weibull AFT preprocessing summary",
	)
	materialize_dataframe_table(
		ctx,
		df=raw_feature_manifest_df,
		table_name=artifact_name(ctx, "table_weibull_aft_raw_feature_manifest"),
		block_number="5.11.2",
		label="Stage 5.11.2 table_weibull_aft_raw_feature_manifest - Weibull AFT raw feature manifest",
	)
	materialize_dataframe_table(
		ctx,
		df=output_feature_manifest_df,
		table_name=artifact_name(ctx, "table_weibull_aft_feature_names_out"),
		block_number="5.11.2",
		label="Stage 5.11.2 table_weibull_aft_feature_names_out - Weibull AFT output feature manifest",
	)
	...
	preprocessing_config = {
		...
		"aft_positioning_note": "early-window comparable Weibull AFT benchmark",
		"window_weeks": int(ctx.effective_window_weeks),
		"duration_floor": float(AFT_DURATION_FLOOR),
	}
	preprocessing_config_path = ctx.metadata_dir / artifact_filename(ctx, "weibull_aft_preprocessing_config.json")
	save_json(preprocessing_config, preprocessing_config_path)
	print_artifact("weibull_aft_preprocessing_config", str(preprocessing_config_path))
	print(preprocessing_config_path.read_text(encoding="utf-8"))

	log_stage_end("5.11.2")
	return AFTTreatmentState(
		train_df=train_df,
		test_df=test_df,
		feature_columns=operational_feature_columns,
		expected_features_raw=expected_features_raw,
...
def build_aft_training_frame(features_df: pd.DataFrame, truth_df: pd.DataFrame) -> pd.DataFrame:
	training_frame = features_df.copy().reset_index(drop=True)
	training_frame["duration"] = np.maximum(truth_df["duration"].astype(float).to_numpy(), AFT_DURATION_FLOOR)
	...
	if not np.isfinite(training_frame.drop(columns=["event"]).to_numpy(dtype=float)).all():
		raise ValueError("The Weibull AFT training frame contains non-finite values.")
	return training_frame


def aft_survival_frame(model: WeibullAFTFitter, features_df: pd.DataFrame, enrollment_ids: list[str], time_grid: np.ndarray) -> pd.DataFrame:
	...
	if survival_wide_df.isna().any().any():
		raise ValueError("The Weibull AFT survival surface contains missing values after prediction.")
	if not np.isfinite(survival_wide_df.to_numpy(dtype=float)).all():
		raise ValueError("The Weibull AFT survival surface contains non-finite values.")
	return survival_wide_df.clip(lower=0.0, upper=1.0)


def fit_weibull_aft_model(model: WeibullAFTFitter, training_df: pd.DataFrame) -> WeibullAFTFitter:
	with warnings.catch_warnings():
		warnings.filterwarnings(
			"ignore",
			message=r".*encountered in matmul",
		...
		model.fit(training_df, duration_col="duration", event_col="event")
	return model
...
def tune_and_evaluate_aft_model(ctx: PipelineContext, treatment: AFTTreatmentState) -> None:
	from dropout_bench_v3_A_2_runtime_config import resolve_runtime_tuning_parallel_backend

	# What this block does:
	# - tunes the Weibull AFT grid on a grouped validation split, refits the best model on full training data, evaluates all benchmark outputs, and persists artifacts.
	# Inputs:
	# - files read: none beyond the fitted preprocessing state already loaded in memory.
	# - DuckDB tables read: no direct reads here; it consumes in-memory treatment matrices derived from enrollment_cox_ready_train/test.
	# - configuration values used: random seed, validation split size, benchmark horizons, calibration bins, and AFT duration floor.
	# Outputs:
	# - files generated: outputs_benchmark_survival/models/weibull_aft_tuned.joblib, outputs_benchmark_survival/models/weibull_aft_preprocessor.joblib, outputs_benchmark_survival/metadata/weibull_aft_tuned_model_config.json.
	# - DuckDB tables created or updated: table_weibull_aft_tuning_results, table_weibull_aft_tuned_test_predictions, table_weibull_aft_tuned_primary_metrics, table_weibull_aft_tuned_brier_by_horizon, table_weibull_aft_tuned_secondary_metrics, table_weibull_aft_tuned_td_auc_support_audit, table_weibull_aft_tuned_row_diagnostics, table_weibull_aft_tuned_support_by_horizon, table_weibull_aft_tuned_calibration_summary, table_weibull_aft_tuned_calibration_bins_by_horizon, table_weibull_aft_tuned_predicted_vs_observed_survival.
	# - objects returned in memory: none; this block persists all final analytical outputs.
	log_stage_start("5.11.3", "Tune and evaluate the comparable-window Weibull AFT model")
	...
	truth_subtrain_df = build_truth_by_enrollment(treatment.train_df.iloc[train_internal_idx], "aft_validation_subtrain")
	truth_val_df = build_truth_by_enrollment(treatment.train_df.iloc[val_internal_idx], "aft_validation_holdout")
	...
	X_subtrain_df = align_feature_frame(
		...
		frame_name="Weibull AFT subtrain feature frame",
	)
	X_val_df = align_feature_frame(
		...
		frame_name="Weibull AFT validation feature frame",
	)
	aft_train_sub_df = build_aft_training_frame(X_subtrain_df, truth_subtrain_df)

	tuning_workers = max(1, min(int(ctx.cpu_cores), len(AFT_TUNING_GRID)))
	...
	tuning_rows = joblib.Parallel(n_jobs=tuning_workers, prefer=tuning_parallel_backend)(
		...
		for candidate in AFT_TUNING_GRID
	)
	best_candidate_row = min(tuning_rows, key=lambda row: (float(row["val_ibs"]), int(row["candidate_id"]))) if tuning_rows else None
	best_candidate = None if best_candidate_row is None else {
		"candidate_id": int(best_candidate_row["candidate_id"]),
		"params": {"penalizer": float(best_candidate_row["penalizer"]), "l1_ratio": float(best_candidate_row["l1_ratio"])},
		"duration_floor": float(AFT_DURATION_FLOOR),
	...
	if best_candidate is None:
		raise RuntimeError("The Weibull AFT tuning grid did not produce a best candidate.")
	...
	X_train_full_df = align_feature_frame(
		...
		frame_name="Weibull AFT full-train feature frame",
	)
	X_test_df = align_feature_frame(
		...
		frame_name="Weibull AFT test feature frame",
	)
	aft_train_full_df = build_aft_training_frame(X_train_full_df, truth_train_df)
	...
	best_model = WeibullAFTFitter(**best_candidate["params"])
	fit_weibull_aft_model(best_model, aft_train_full_df)

	aft_time_grid = np.arange(0, int(max(truth_test_df["duration"].max(), max(BENCHMARK_HORIZONS))) + 1, dtype=float)
	survival_wide_df = aft_survival_frame(best_model, X_test_df, ordered_test_enrollment_ids, aft_time_grid)
	if survival_wide_df.isna().any().any():
		raise ValueError("The tuned Weibull AFT survival surface contains missing values after enrollment alignment.")
	...
	for horizon_week in BENCHMARK_HORIZONS:
		...
		if horizon_predictions_df[["pred_survival_h", "pred_risk_h"]].isna().any().any():
			raise ValueError(f"The tuned Weibull AFT predictions contain missing values at horizon {horizon_week}.")
	...
	if not np.isfinite(max_ipcw_time):
		raise ValueError("Could not determine a valid censoring-support horizon for the comparable Weibull AFT model.")
		...
		td_auc_audit_rows.append(
			{
				...
				"notes": "IPCW-supported subset used for Weibull AFT time-dependent AUC",
			}
		)
	...
	row_pred_survival = np.asarray(
		[float(survival_wide_df.loc[int(duration), enrollment_id]) for enrollment_id, duration in zip(truth_test_df["enrollment_id"], truth_test_df["duration"])],
		dtype=float,
	)
	row_pred_risk = 1.0 - row_pred_survival
	if not np.isfinite(row_pred_risk).all():
		raise ValueError("The tuned Weibull AFT event-time risk diagnostics contain non-finite values.")
	if truth_test_df["event"].nunique() < 2:
		raise ValueError("The Weibull AFT test truth does not contain both event classes for row-level diagnostics.")
	...
	model_path = ctx.models_dir / artifact_filename(ctx, "weibull_aft_tuned.joblib")
	preprocessor_path = ctx.models_dir / artifact_filename(ctx, "weibull_aft_preprocessor.joblib")
	config_path = ctx.metadata_dir / artifact_filename(ctx, "weibull_aft_tuned_model_config.json")
	joblib.dump(best_model, model_path)
	joblib.dump(treatment.preprocessor, preprocessor_path)

	materialize_dataframe_table(ctx, tuning_results_df, artifact_name(ctx, "table_weibull_aft_tuning_results"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuning_results - Weibull AFT tuning results")
	materialize_dataframe_table(ctx, test_predictions_df, artifact_name(ctx, "table_weibull_aft_tuned_test_predictions"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_test_predictions - Weibull AFT test predictions")
	materialize_dataframe_table(ctx, primary_metrics_df, artifact_name(ctx, "table_weibull_aft_tuned_primary_metrics"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_primary_metrics - Weibull AFT primary metrics")
	materialize_dataframe_table(ctx, brier_by_horizon_df, artifact_name(ctx, "table_weibull_aft_tuned_brier_by_horizon"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_brier_by_horizon - Weibull AFT Brier scores by horizon")
	materialize_dataframe_table(ctx, secondary_metrics_df, artifact_name(ctx, "table_weibull_aft_tuned_secondary_metrics"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_secondary_metrics - Weibull AFT secondary metrics")
	materialize_dataframe_table(ctx, td_auc_support_audit_df, artifact_name(ctx, "table_weibull_aft_tuned_td_auc_support_audit"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_td_auc_support_audit - Weibull AFT IPCW support audit")
	materialize_dataframe_table(ctx, row_diagnostics_df, artifact_name(ctx, "table_weibull_aft_tuned_row_diagnostics"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_row_diagnostics - Weibull AFT row-level diagnostics")
	materialize_dataframe_table(ctx, support_by_horizon_df, artifact_name(ctx, "table_weibull_aft_tuned_support_by_horizon"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_support_by_horizon - Weibull AFT support by horizon")
	materialize_dataframe_table(ctx, calibration_summary_df, artifact_name(ctx, "table_weibull_aft_tuned_calibration_summary"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_calibration_summary - Weibull AFT calibration summary")
	materialize_dataframe_table(ctx, calibration_bins_df, artifact_name(ctx, "table_weibull_aft_tuned_calibration_bins_by_horizon"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_calibration_bins_by_horizon - Weibull AFT calibration bins")
	materialize_dataframe_table(ctx, predicted_vs_observed_survival_df, artifact_name(ctx, "table_weibull_aft_tuned_predicted_vs_observed_survival"), "5.11.3", "Stage 5.11.3 table_weibull_aft_tuned_predicted_vs_observed_survival - Weibull AFT predicted versus observed survival")
	...
	config_payload = {
		...
		"input_train_table": ctx.input_train_table,
		"input_test_table": ctx.input_test_table,
		"search_space": [{"candidate_id": int(candidate["candidate_id"]), "penalizer": float(candidate["penalizer"]), "l1_ratio": float(candidate["l1_ratio"])} for candidate in AFT_TUNING_GRID],
		...
		"design_note": (
			"The comparable Weibull AFT arm preserves the canonical early-window contract, "
			"uses the same deterministic preprocessing as the other continuous-time baselines, "
			"and applies an explicit positive-duration floor only inside the model-local AFT training frames."
		),
	}
	save_json(config_payload, config_path)
	print_artifact("weibull_aft_model", str(model_path))
	print_artifact("weibull_aft_preprocessor", str(preprocessor_path))
	print_artifact("weibull_aft_tuned_model_config", str(config_path))
	print(config_path.read_text(encoding="utf-8"))
	print(pd.DataFrame([best_candidate]).to_string(index=False))

	log_stage_end("5.11.3")


def close_context(ctx: PipelineContext) -> None:
	# What this block does:
	# - closes the DuckDB runtime and prints the final artifact closure marker.
	# Inputs:
	# - files read: none.
	# - DuckDB tables read: none.
	# - configuration values used: resolved DuckDB path from the runtime context.
	# Outputs:
	# - files generated: none.
	# - DuckDB tables created or updated: none.
	# - objects returned in memory: none; the open connection is closed.
	from util import close_duckdb_connection

	log_stage_start("5.11.4", "Close runtime resources")
	close_duckdb_connection(ctx.con)
	print_artifact("duckdb_connection", f"closed::{ctx.duckdb_path}")
	log_stage_end("5.11.4")
...
def run_single_window(window_weeks: int) -> None:
	...
	try:
		...
		treatment = build_aft_treatment(ctx)
		tune_and_evaluate_aft_model(ctx, treatment)
	finally:
		if ctx is not None:
			close_context(ctx)
