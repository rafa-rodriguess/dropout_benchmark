...
"""
Production weighted neural discrete-time hazard benchmark module for Item 6 sensitivity.
...
Expected outputs:
- DuckDB tables table_p18_neural_weighted_treatment_summary, table_p18_neural_weighted_feature_manifest,
  table_p18_neural_weighted_canonical_alignment, table_p18_neural_weighted_output_feature_manifest,
  table_neural_weighted_tuning_results, table_neural_weighted_tuned_training_history,
  table_neural_weighted_tuned_test_predictions, table_neural_weighted_tuned_primary_metrics,
  table_neural_weighted_tuned_brier_by_horizon, table_neural_weighted_tuned_secondary_metrics,
  table_neural_weighted_tuned_td_auc_support_audit, table_neural_weighted_tuned_row_diagnostics,
  table_neural_weighted_tuned_support_by_horizon, table_neural_weighted_tuned_calibration_summary,
  table_neural_weighted_tuned_calibration_bins_by_horizon,
  table_neural_weighted_tuned_predicted_vs_observed_survival,
  table_neural_weighted_tuned_hazard_audit_summary, table_neural_weighted_tuned_hazard_by_week
- outputs_benchmark_survival/metadata/metadata_p18_neural_weighted_treatment.json
- outputs_benchmark_survival/metadata/neural_weighted_tuned_model_config.json
- outputs_benchmark_survival/models/neural_discrete_time_survival_weighted_tuned.pt
- outputs_benchmark_survival/models/neural_discrete_time_weighted_preprocessor.joblib
...
- table_p18_neural_weighted_treatment_summary
- table_p18_neural_weighted_feature_manifest
- table_p18_neural_weighted_canonical_alignment
- table_p18_neural_weighted_output_feature_manifest
- table_neural_weighted_tuning_results
- table_neural_weighted_tuned_training_history
- table_neural_weighted_tuned_test_predictions
- table_neural_weighted_tuned_primary_metrics
- table_neural_weighted_tuned_brier_by_horizon
- table_neural_weighted_tuned_secondary_metrics
- table_neural_weighted_tuned_td_auc_support_audit
- table_neural_weighted_tuned_row_diagnostics
- table_neural_weighted_tuned_support_by_horizon
- table_neural_weighted_tuned_calibration_summary
- table_neural_weighted_tuned_calibration_bins_by_horizon
- table_neural_weighted_tuned_predicted_vs_observed_survival
- table_neural_weighted_tuned_hazard_audit_summary
- table_neural_weighted_tuned_hazard_by_week
- pipeline_table_catalog
...
NOTEBOOK_NAME = "dropout_bench_v3_D_5_3_weighted.ipynb"
PREVIEW_ROWS = 20
...
MODEL_NAME = "neural_discrete_time_survival_weighted_tuned"
...
def build_neural_treatment(ctx: PipelineContext, run_spec: WindowRunSpec) -> NeuralTreatmentState:
    ...
    # - DuckDB tables table_p18_neural_weighted_treatment_summary, table_p18_neural_weighted_feature_manifest,
    #   table_p18_neural_weighted_canonical_alignment, table_p18_neural_weighted_output_feature_manifest
    # - metadata artifact metadata_p18_neural_weighted_treatment.json
    ...
    materialize_dataframe_table(
        ...
        table_name=apply_name_suffix("table_p18_neural_weighted_treatment_summary", run_spec.output_suffix),
        block_number="5.3.2",
        label=f"Stage 5.3.2 {apply_name_suffix('table_p18_neural_weighted_treatment_summary', run_spec.output_suffix)} — Neural treatment summary",
    )
    materialize_dataframe_table(
        ctx,
        df=feature_manifest_df,
        table_name=apply_name_suffix("table_p18_neural_weighted_feature_manifest", run_spec.output_suffix),
        block_number="5.3.2",
        label=f"Stage 5.3.2 {apply_name_suffix('table_p18_neural_weighted_feature_manifest', run_spec.output_suffix)} — Neural feature manifest",
    )
    materialize_dataframe_table(
        ctx,
        df=canonical_alignment_df,
        table_name=apply_name_suffix("table_p18_neural_weighted_canonical_alignment", run_spec.output_suffix),
        block_number="5.3.2",
        label=f"Stage 5.3.2 {apply_name_suffix('table_p18_neural_weighted_canonical_alignment', run_spec.output_suffix)} — Canonical feature alignment",
    )
    materialize_dataframe_table(
        ctx,
        df=output_feature_manifest_df,
        table_name=apply_name_suffix("table_p18_neural_weighted_output_feature_manifest", run_spec.output_suffix),
        block_number="5.3.2",
        label=f"Stage 5.3.2 {apply_name_suffix('table_p18_neural_weighted_output_feature_manifest', run_spec.output_suffix)} — Preprocessed feature manifest",
    )

    treatment_metadata_path = ctx.metadata_dir / append_suffix_before_extension(
        "metadata_p18_neural_weighted_treatment.json",
...
def fit_neural_candidate(
    ...
    positive_count = max(float(np.sum(y_train_internal)), 1.0)
    negative_count = max(float(y_train_internal.shape[0] - positive_count), 1.0)
...
def tune_and_evaluate_neural_model(ctx: PipelineContext, treatment: NeuralTreatmentState, run_spec: WindowRunSpec) -> None:
    ...
    # - neural_discrete_time_survival_weighted_tuned.pt
    # - neural_discrete_time_weighted_preprocessor.joblib
    # - neural_weighted_tuned_model_config.json
        ...
        predicted_vs_observed_rows.append(
            {
                ...
                "abs_gap": float(
                    abs(evaluable_df["pred_survival_h"].mean() - evaluable_df["observed_survival_by_h"].mean())
                ),
            }
        )
    ...
    materialize_dataframe_table(
        ...
        table_name=apply_name_suffix("table_neural_weighted_tuning_results", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuning_results', run_spec.output_suffix)} — Neural tuning results",
    )
    materialize_dataframe_table(
        ctx,
        df=history_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_training_history", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_training_history', run_spec.output_suffix)} — Neural best-candidate training summary",
    )
    materialize_dataframe_table(
        ctx,
        df=test_predictions_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_test_predictions", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_test_predictions', run_spec.output_suffix)} — Neural test predictions",
    )
    materialize_dataframe_table(
        ctx,
        df=primary_metrics_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_primary_metrics", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_primary_metrics', run_spec.output_suffix)} — Neural primary metrics",
    )
    materialize_dataframe_table(
        ctx,
        df=brier_by_horizon_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_brier_by_horizon", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_brier_by_horizon', run_spec.output_suffix)} — Neural Brier scores by horizon",
    )
    materialize_dataframe_table(
        ctx,
        df=secondary_metrics_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_secondary_metrics", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_secondary_metrics', run_spec.output_suffix)} — Neural secondary metrics",
    )
    materialize_dataframe_table(
        ctx,
        df=td_auc_support_audit_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_td_auc_support_audit", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_td_auc_support_audit', run_spec.output_suffix)} — Neural IPCW support audit",
    )
    materialize_dataframe_table(
        ctx,
        df=row_diagnostics_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_row_diagnostics", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_row_diagnostics', run_spec.output_suffix)} — Neural row-level diagnostics",
    )
    materialize_dataframe_table(
        ctx,
        df=support_by_horizon_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_support_by_horizon", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_support_by_horizon', run_spec.output_suffix)} — Neural support by horizon",
    )
    materialize_dataframe_table(
        ctx,
        df=calibration_summary_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_calibration_summary", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_calibration_summary', run_spec.output_suffix)} — Neural calibration summary",
    )
    materialize_dataframe_table(
        ctx,
        df=calibration_bins_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_calibration_bins_by_horizon", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_calibration_bins_by_horizon', run_spec.output_suffix)} — Neural calibration bins",
    )
    materialize_dataframe_table(
        ctx,
        df=predicted_vs_observed_survival_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_predicted_vs_observed_survival", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_predicted_vs_observed_survival', run_spec.output_suffix)} — Neural predicted versus observed survival",
    )
    materialize_dataframe_table(
        ctx,
        df=hazard_audit_summary_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_hazard_audit_summary", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_hazard_audit_summary', run_spec.output_suffix)} — Neural hazard audit summary",
    )
    materialize_dataframe_table(
        ctx,
        df=hazard_by_week_df,
        table_name=apply_name_suffix("table_neural_weighted_tuned_hazard_by_week", run_spec.output_suffix),
        block_number="5.3.3",
        label=f"Stage 5.3.3 {apply_name_suffix('table_neural_weighted_tuned_hazard_by_week', run_spec.output_suffix)} — Neural hazard by week",
    )
    ...
    model_config = {
        ...
        "optimizer": "Adam",
        "loss": "BCEWithLogitsLoss(pos_weight=guarded_positive_ratio)",
        ...
        "weighting_strategy": "BCEWithLogitsLoss uses a positive-class weight derived from the guarded subtrain imbalance ratio for each candidate.",
        "design_note": "Enrollment truth and evaluation alignment are derived directly from the neural person-period tables so the Item 6 weighted variant changes only the fitting loss, not the evaluation contract.",
    }
