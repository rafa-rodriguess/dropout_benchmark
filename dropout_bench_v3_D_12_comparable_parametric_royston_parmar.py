...
"""
Production comparable-window Royston-Parmar benchmark module for stage D5.12.

What this file does:
- prepares the comparable-window flexible parametric treatment directly from DuckDB-ready enrollment tables
- tunes and fits the official Royston-Parmar style spline AFT model under the benchmark protocol
...
- materialize the full comparable flexible parametric benchmark arm deterministically from DuckDB-ready tables without notebook runtime state
...
Expected outputs:
- DuckDB tables table_royston_parmar_preprocessing_summary, table_royston_parmar_raw_feature_manifest,
  table_royston_parmar_feature_names_out, table_royston_parmar_formula_feature_map,
  table_royston_parmar_tuning_results, table_royston_parmar_tuned_test_predictions,
  table_royston_parmar_tuned_primary_metrics, table_royston_parmar_tuned_brier_by_horizon,
  table_royston_parmar_tuned_secondary_metrics, table_royston_parmar_tuned_td_auc_support_audit,
  table_royston_parmar_tuned_row_diagnostics, table_royston_parmar_tuned_support_by_horizon,
  table_royston_parmar_tuned_calibration_summary,
  table_royston_parmar_tuned_calibration_bins_by_horizon,
  table_royston_parmar_tuned_predicted_vs_observed_survival
- outputs_benchmark_survival/metadata/royston_parmar_preprocessing_config.json
- outputs_benchmark_survival/metadata/royston_parmar_tuned_model_config.json
- outputs_benchmark_survival/models/royston_parmar_tuned.joblib
- outputs_benchmark_survival/models/royston_parmar_preprocessor.joblib
...
- table_royston_parmar_preprocessing_summary
- table_royston_parmar_raw_feature_manifest
- table_royston_parmar_feature_names_out
- table_royston_parmar_formula_feature_map
- table_royston_parmar_tuning_results
- table_royston_parmar_tuned_test_predictions
- table_royston_parmar_tuned_primary_metrics
- table_royston_parmar_tuned_brier_by_horizon
- table_royston_parmar_tuned_secondary_metrics
- table_royston_parmar_tuned_td_auc_support_audit
- table_royston_parmar_tuned_row_diagnostics
- table_royston_parmar_tuned_support_by_horizon
- table_royston_parmar_tuned_calibration_summary
- table_royston_parmar_tuned_calibration_bins_by_horizon
- table_royston_parmar_tuned_predicted_vs_observed_survival
- pipeline_table_catalog
...
- invalid preprocessing outputs, unsupported evaluation subsets, convergence warnings, or non-finite model outputs raise immediately
...
NOTEBOOK_NAME = "dropout_bench_D_v5_12_royston_parmar.ipynb"
PREVIEW_ROWS = 20
...
POSITIVE_DURATION_FLOOR = 1.0
MODEL_NAME = "royston_parmar_tuned"
MODEL_IMPLEMENTATION = "lifelines_crc_spline_aft"
...
CRC_SPLINE_TUNING_GRID = (
    {"candidate_id": 1,  "n_baseline_knots": 2, "penalizer": 0.01,  "l1_ratio": 0.0},
    {"candidate_id": 2,  "n_baseline_knots": 2, "penalizer": 0.025, "l1_ratio": 0.0},
    {"candidate_id": 3,  "n_baseline_knots": 2, "penalizer": 0.05,  "l1_ratio": 0.0},
    {"candidate_id": 4,  "n_baseline_knots": 2, "penalizer": 0.10,  "l1_ratio": 0.0},
    {"candidate_id": 5,  "n_baseline_knots": 2, "penalizer": 0.15,  "l1_ratio": 0.0},
    {"candidate_id": 6,  "n_baseline_knots": 2, "penalizer": 0.20,  "l1_ratio": 0.0},
    {"candidate_id": 7,  "n_baseline_knots": 2, "penalizer": 0.30,  "l1_ratio": 0.0},
    {"candidate_id": 8,  "n_baseline_knots": 2, "penalizer": 0.50,  "l1_ratio": 0.0},
    {"candidate_id": 9,  "n_baseline_knots": 2, "penalizer": 0.70,  "l1_ratio": 0.0},
    {"candidate_id": 10, "n_baseline_knots": 2, "penalizer": 1.0,   "l1_ratio": 0.0},
    {"candidate_id": 11, "n_baseline_knots": 2, "penalizer": 2.0,   "l1_ratio": 0.0},
    {"candidate_id": 12, "n_baseline_knots": 2, "penalizer": 5.0,   "l1_ratio": 0.0},
    {"candidate_id": 13, "n_baseline_knots": 3, "penalizer": 0.01,  "l1_ratio": 0.0},
    {"candidate_id": 14, "n_baseline_knots": 3, "penalizer": 0.025, "l1_ratio": 0.0},
    {"candidate_id": 15, "n_baseline_knots": 3, "penalizer": 0.05,  "l1_ratio": 0.0},
    {"candidate_id": 16, "n_baseline_knots": 3, "penalizer": 0.10,  "l1_ratio": 0.0},
    {"candidate_id": 17, "n_baseline_knots": 3, "penalizer": 0.15,  "l1_ratio": 0.0},
    {"candidate_id": 18, "n_baseline_knots": 3, "penalizer": 0.20,  "l1_ratio": 0.0},
    {"candidate_id": 19, "n_baseline_knots": 3, "penalizer": 0.30,  "l1_ratio": 0.0},
    {"candidate_id": 20, "n_baseline_knots": 3, "penalizer": 0.50,  "l1_ratio": 0.0},
    {"candidate_id": 21, "n_baseline_knots": 3, "penalizer": 0.70,  "l1_ratio": 0.0},
    {"candidate_id": 22, "n_baseline_knots": 3, "penalizer": 1.0,   "l1_ratio": 0.0},
    {"candidate_id": 23, "n_baseline_knots": 3, "penalizer": 2.0,   "l1_ratio": 0.0},
    {"candidate_id": 24, "n_baseline_knots": 3, "penalizer": 5.0,   "l1_ratio": 0.0},
)
...
class SplineTreatmentState:
    ...
    formula_feature_map: dict[str, str]
...
class LowVarianceFilter(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: Any, y: Any = None) -> "LowVarianceFilter":
        ...
        if not bool(np.any(self.support_mask_)):
            raise ValueError("All transformed Royston-Parmar features were removed by the low-variance filter.")
        return self
...
def materialize_dataframe_table(
    ...
    temp_view_name = "__stage_d_5_12_materialize_df__"
    try:
...
def require_tables(con: Any, required_tables: list[str], block_number: str) -> None:
    ...
    if missing_tables:
        raise FileNotFoundError(f"{block_number}: missing required DuckDB table(s): {', '.join(missing_tables)}")
...
def build_truth_by_enrollment(df: pd.DataFrame, dataset_name: str) -> pd.DataFrame:
    ...
    inconsistent_duration_ids = grouped_df.loc[grouped_df["duration_min"] != grouped_df["duration_max"], "enrollment_id"]
...
def make_formula_safe_feature_names(feature_names: list[str]) -> dict[str, str]:
    mapping: dict[str, str] = {}
    used_names: set[str] = set()
    for position, original_name in enumerate(feature_names):
        safe_name = re.sub(r"[^0-9A-Za-z_]", "_", str(original_name))
        safe_name = re.sub(r"_+", "_", safe_name).strip("_")
        if not safe_name:
            safe_name = f"feature_{position}"
        ...
        while candidate_name in used_names:
            suffix += 1
            candidate_name = f"{safe_name}_{suffix}"
        used_names.add(candidate_name)
        mapping[str(original_name)] = candidate_name
    return mapping
...
def initialize_context(window_weeks: int) -> PipelineContext:
    ...
    log_stage_start("5.12.1", "Lightweight runtime bootstrap")
    ...
    require_tables(con, [input_train_table, input_test_table], block_number="5.12.1")
    ...
    log_stage_end("5.12.1")
    return ctx


def build_spline_treatment(ctx: PipelineContext) -> SplineTreatmentState:
    log_stage_start("5.12.2", "Build comparable-window Royston-Parmar treatment")
    ...
    if sorted(expected_comparable_numeric_features) != sorted(numeric_features):
        raise ValueError(
            "The comparable Royston-Parmar numeric feature specification does not match the canonical modeling contract. "
    ...
    train_df = load_required_table(ctx, ctx.input_train_table, required_columns, block_number="5.12.2")
    test_df = load_required_table(ctx, ctx.input_test_table, required_columns, block_number="5.12.2")
    ...
    X_train = np.asarray(preprocessor.fit_transform(train_df[operational_feature_columns]), dtype=np.float64)
    X_test = np.asarray(preprocessor.transform(test_df[operational_feature_columns]), dtype=np.float64)
    if X_train.ndim != 2 or X_test.ndim != 2:
        raise ValueError(f"The comparable Royston-Parmar transformed matrices must be 2D. Got train={X_train.shape}, test={X_test.shape}.")
    if not np.isfinite(X_train).all() or not np.isfinite(X_test).all():
        raise ValueError("The comparable Royston-Parmar transformed matrices contain non-finite values.")
    ...
    formula_feature_map = make_formula_safe_feature_names(feature_names_out)
    feature_names_safe = [formula_feature_map[feature_name] for feature_name in feature_names_out]
        ...
        [
            {
                ...
                "n_features_after_formula_sanitization": int(len(feature_names_safe)),
                ...
                "categorical_encoding": "one_hot_drop_first_handle_unknown_ignore",
                "numeric_scaling": "standard_scaler_fit_on_train_only",
                "variance_filter_threshold": float(LOW_VARIANCE_THRESHOLD),
                "output_dtype": "float64",
                "positioning_note": "early-window comparable flexible parametric benchmark",
    ...
    output_feature_manifest_df = pd.DataFrame(
        {"feature_name_out": feature_names_out, "transformed_variance_train": retained_variances}
    )
    formula_feature_map_df = pd.DataFrame(
        {
            "feature_name_out": feature_names_out,
            "feature_name_safe": feature_names_safe,
        }
    )
    ...
    materialize_dataframe_table(
        ...
        table_name=artifact_name(ctx, "table_royston_parmar_preprocessing_summary"),
        block_number="5.12.2",
        label="Stage 5.12.2 table_royston_parmar_preprocessing_summary - Royston-Parmar preprocessing summary",
    )
    materialize_dataframe_table(
        ctx,
        df=raw_feature_manifest_df,
        table_name=artifact_name(ctx, "table_royston_parmar_raw_feature_manifest"),
        block_number="5.12.2",
        label="Stage 5.12.2 table_royston_parmar_raw_feature_manifest - Royston-Parmar raw feature manifest",
    )
    materialize_dataframe_table(
        ctx,
        df=output_feature_manifest_df,
        table_name=artifact_name(ctx, "table_royston_parmar_feature_names_out"),
        block_number="5.12.2",
        label="Stage 5.12.2 table_royston_parmar_feature_names_out - Royston-Parmar output feature manifest",
    )
    materialize_dataframe_table(
        ctx,
        df=formula_feature_map_df,
        table_name=artifact_name(ctx, "table_royston_parmar_formula_feature_map"),
        block_number="5.12.2",
        label="Stage 5.12.2 table_royston_parmar_formula_feature_map - Royston-Parmar formula feature map",
    )
    ...
    preprocessing_config = {
        ...
        "formula_feature_map": formula_feature_map,
        "dropped_low_variance_features": dropped_feature_names,
        "positioning_note": "early-window comparable flexible parametric benchmark",
        "window_weeks": int(ctx.effective_window_weeks),
        "duration_floor": float(POSITIVE_DURATION_FLOOR),
    }
    preprocessing_config_path = ctx.metadata_dir / artifact_filename(ctx, "royston_parmar_preprocessing_config.json")
    save_json(preprocessing_config, preprocessing_config_path)
    print_artifact("royston_parmar_preprocessing_config", str(preprocessing_config_path))
    print(preprocessing_config_path.read_text(encoding="utf-8"))

    log_stage_end("5.12.2")
    return SplineTreatmentState(
        train_df=train_df,
        test_df=test_df,
        feature_columns=operational_feature_columns,
        expected_features_raw=expected_features_raw,
...
def align_feature_frame(
    ...
    if len(raw_enrollment_ids) != len(set(raw_enrollment_ids)):
        raise ValueError(f"{frame_name} contains duplicate enrollment_id values and cannot be aligned deterministically.")
    ...
    if aligned_df.isna().any().any():
        raise ValueError(f"{frame_name} contains missing values after enrollment alignment.")
    return aligned_df.astype(float)


def build_training_frame(features_df: pd.DataFrame, truth_df: pd.DataFrame) -> pd.DataFrame:
    training_frame = features_df.copy().reset_index(drop=True)
    training_frame["duration"] = np.maximum(truth_df["duration"].astype(float).to_numpy(), POSITIVE_DURATION_FLOOR)
    training_frame["event"] = truth_df["event"].astype(int).to_numpy()
    if not np.isfinite(training_frame.drop(columns=["event"]).to_numpy(dtype=float)).all():
        raise ValueError("The Royston-Parmar training frame contains non-finite values.")
    return training_frame


def build_crc_regressors(feature_names_safe: list[str], n_baseline_knots: int) -> dict[str, str]:
    if not feature_names_safe:
        raise ValueError("At least one transformed feature is required to build the Royston-Parmar regressors.")
    regressors = {"beta_": " + ".join(feature_names_safe)}
    for knot_index in range(int(n_baseline_knots)):
        regressors[f"gamma{knot_index}_"] = "1"
    return regressors


def crc_survival_frame(model: CRCSplineFitter, features_df: pd.DataFrame, enrollment_ids: list[str], time_grid: np.ndarray) -> pd.DataFrame:
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        survival_wide_df = model.predict_survival_function(features_df, times=time_grid)
    survival_wide_df.columns = enrollment_ids
    survival_wide_df.columns.name = "enrollment_id"
    survival_wide_df.index = pd.Index(time_grid.astype(int), name="week")
    if survival_wide_df.isna().any().any():
        raise ValueError("The Royston-Parmar survival surface contains missing values after prediction.")
    if not np.isfinite(survival_wide_df.to_numpy(dtype=float)).all():
        raise ValueError("The Royston-Parmar survival surface contains non-finite values.")
    return survival_wide_df.clip(lower=0.0, upper=1.0)


def fit_crc_spline_model(model: CRCSplineFitter, training_df: pd.DataFrame, regressors: dict[str, str]) -> CRCSplineFitter:
...
def tune_and_evaluate_spline_model(ctx: PipelineContext, treatment: SplineTreatmentState) -> None:
    from dropout_bench_v3_A_2_runtime_config import resolve_runtime_tuning_parallel_backend

    log_stage_start("5.12.3", "Tune and evaluate the comparable-window Royston-Parmar model")
    ...
    group_splitter = GroupShuffleSplit(n_splits=1, test_size=VALIDATION_TEST_SIZE, random_state=ctx.random_seed)
    ...
    truth_subtrain_df = build_truth_by_enrollment(treatment.train_df.iloc[train_internal_idx], "royston_parmar_validation_subtrain")
    truth_val_df = build_truth_by_enrollment(treatment.train_df.iloc[val_internal_idx], "royston_parmar_validation_holdout")

    X_subtrain_df = align_feature_frame(
        treatment.X_train[train_internal_idx],
        treatment.feature_names_out,
        subtrain_raw_ids,
        truth_subtrain_df["enrollment_id"].astype(str).tolist(),
        frame_name="Royston-Parmar subtrain feature frame",
    ).rename(columns=treatment.formula_feature_map)
    X_val_df = align_feature_frame(
        treatment.X_train[val_internal_idx],
        treatment.feature_names_out,
        val_raw_ids,
        truth_val_df["enrollment_id"].astype(str).tolist(),
        frame_name="Royston-Parmar validation feature frame",
    ).rename(columns=treatment.formula_feature_map)
    spline_train_sub_df = build_training_frame(X_subtrain_df, truth_subtrain_df)

    tuning_workers = max(1, min(int(ctx.cpu_cores), len(CRC_SPLINE_TUNING_GRID)))
    ...
    def evaluate_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
        regressors = build_crc_regressors(treatment.feature_names_safe, int(candidate["n_baseline_knots"]))
        candidate_row: dict[str, Any] = {
            "candidate_id": int(candidate["candidate_id"]),
            "n_baseline_knots": int(candidate["n_baseline_knots"]),
        ...
        try:
            candidate_model = CRCSplineFitter(
                n_baseline_knots=int(candidate["n_baseline_knots"]),
                penalizer=float(candidate["penalizer"]),
                l1_ratio=float(candidate["l1_ratio"]),
            )
            fit_crc_spline_model(candidate_model, spline_train_sub_df, regressors=regressors)
            val_time_grid = np.arange(1, int(max(truth_val_df["duration"].max(), max(BENCHMARK_HORIZONS))) + 1, dtype=float)
            val_survival_df = crc_survival_frame(
                candidate_model,
                X_val_df,
                truth_val_df["enrollment_id"].astype(str).tolist(),
                val_time_grid,
            ...
            candidate_row["fit_status"] = "ok"
            candidate_row["val_ibs"] = float(val_ibs)
            candidate_row["val_c_index"] = float(val_c_index)
            candidate_row["error_message"] = None
        except Exception as exc:
            candidate_row["fit_status"] = "failed"
            candidate_row["val_ibs"] = np.nan
            candidate_row["val_c_index"] = np.nan
            candidate_row["error_message"] = f"{type(exc).__name__}: {exc}"
    ...
    tuning_rows = joblib.Parallel(n_jobs=tuning_workers, prefer=tuning_parallel_backend)(
        ...
        for candidate in CRC_SPLINE_TUNING_GRID
    )
    valid_rows = [row for row in tuning_rows if row.get("fit_status") == "ok"]
    best_candidate_row = min(valid_rows, key=lambda row: (float(row["val_ibs"]), int(row["candidate_id"]))) if valid_rows else None
    if best_candidate_row is None:
        raise RuntimeError("The Royston-Parmar tuning grid did not produce a valid candidate without warnings or fit failures.")

    best_candidate = {
        "candidate_id": int(best_candidate_row["candidate_id"]),
        "params": {
            "n_baseline_knots": int(best_candidate_row["n_baseline_knots"]),
            "penalizer": float(best_candidate_row["penalizer"]),
            "l1_ratio": float(best_candidate_row["l1_ratio"]),
        },
        "duration_floor": float(POSITIVE_DURATION_FLOOR),
        "regressors": best_candidate_row["regressors"],
        "val_ibs": float(best_candidate_row["val_ibs"]),
        "val_c_index": float(best_candidate_row["val_c_index"]),
        "implementation": MODEL_IMPLEMENTATION,
    }

    tuning_results_df = pd.DataFrame(tuning_rows).sort_values(
        ["fit_status", "val_ibs", "candidate_id"],
        ascending=[True, True, True],
        kind="mergesort",
        na_position="last",
    ).reset_index(drop=True)

    X_train_full_df = align_feature_frame(
        treatment.X_train,
        treatment.feature_names_out,
        treatment.train_df["enrollment_id"].astype(str).tolist(),
        ordered_train_enrollment_ids,
        frame_name="Royston-Parmar full-train feature frame",
    ).rename(columns=treatment.formula_feature_map)
    X_test_df = align_feature_frame(
        treatment.X_test,
        treatment.feature_names_out,
        treatment.test_df["enrollment_id"].astype(str).tolist(),
        ordered_test_enrollment_ids,
        frame_name="Royston-Parmar test feature frame",
    ).rename(columns=treatment.formula_feature_map)
    spline_train_full_df = build_training_frame(X_train_full_df, truth_train_df)
    ...
    best_model = CRCSplineFitter(**best_candidate["params"])
    fit_crc_spline_model(best_model, spline_train_full_df, regressors=best_candidate["regressors"])

    spline_time_grid = np.arange(1, int(max(truth_test_df["duration"].max(), max(BENCHMARK_HORIZONS))) + 1, dtype=float)
    survival_wide_df = crc_survival_frame(best_model, X_test_df, ordered_test_enrollment_ids, spline_time_grid)
    if survival_wide_df.isna().any().any():
        raise ValueError("The tuned Royston-Parmar survival surface contains missing values after enrollment alignment.")
    ...
    for horizon_week in BENCHMARK_HORIZONS:
        ...
        if horizon_predictions_df[["pred_survival_h", "pred_risk_h"]].isna().any().any():
            raise ValueError(f"The tuned Royston-Parmar predictions contain missing values at horizon {horizon_week}.")
    ...
    max_ipcw_time = float(np.nanmin([float(truth_test_df.loc[truth_test_df["event"] == 0, "duration"].max()), float(spline_time_grid.max())]))
    if not np.isfinite(max_ipcw_time):
        raise ValueError("Could not determine a valid censoring-support horizon for the comparable Royston-Parmar model.")
        ...
        td_auc_audit_rows.append(
            {
                ...
                "notes": "IPCW-supported subset used for Royston-Parmar time-dependent AUC",
            }
        )
    ...
    row_pred_survival = np.asarray(
        [
            float(survival_wide_df.loc[int(max(float(duration), POSITIVE_DURATION_FLOOR)), enrollment_id])
    ...
    if not np.isfinite(row_pred_risk).all():
        raise ValueError("The tuned Royston-Parmar event-time risk diagnostics contain non-finite values.")
    if truth_test_df["event"].nunique() < 2:
        raise ValueError("The Royston-Parmar test truth does not contain both event classes for row-level diagnostics.")
    ...
    model_path = ctx.models_dir / artifact_filename(ctx, "royston_parmar_tuned.joblib")
    preprocessor_path = ctx.models_dir / artifact_filename(ctx, "royston_parmar_preprocessor.joblib")
    config_path = ctx.metadata_dir / artifact_filename(ctx, "royston_parmar_tuned_model_config.json")
    joblib.dump(best_model, model_path)
    joblib.dump(treatment.preprocessor, preprocessor_path)

    materialize_dataframe_table(ctx, tuning_results_df, artifact_name(ctx, "table_royston_parmar_tuning_results"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuning_results - Royston-Parmar tuning results")
    materialize_dataframe_table(ctx, test_predictions_df, artifact_name(ctx, "table_royston_parmar_tuned_test_predictions"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_test_predictions - Royston-Parmar test predictions")
    materialize_dataframe_table(ctx, primary_metrics_df, artifact_name(ctx, "table_royston_parmar_tuned_primary_metrics"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_primary_metrics - Royston-Parmar primary metrics")
    materialize_dataframe_table(ctx, brier_by_horizon_df, artifact_name(ctx, "table_royston_parmar_tuned_brier_by_horizon"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_brier_by_horizon - Royston-Parmar Brier scores by horizon")
    materialize_dataframe_table(ctx, secondary_metrics_df, artifact_name(ctx, "table_royston_parmar_tuned_secondary_metrics"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_secondary_metrics - Royston-Parmar secondary metrics")
    materialize_dataframe_table(ctx, td_auc_support_audit_df, artifact_name(ctx, "table_royston_parmar_tuned_td_auc_support_audit"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_td_auc_support_audit - Royston-Parmar IPCW support audit")
    materialize_dataframe_table(ctx, row_diagnostics_df, artifact_name(ctx, "table_royston_parmar_tuned_row_diagnostics"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_row_diagnostics - Royston-Parmar row-level diagnostics")
    materialize_dataframe_table(ctx, support_by_horizon_df, artifact_name(ctx, "table_royston_parmar_tuned_support_by_horizon"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_support_by_horizon - Royston-Parmar support by horizon")
    materialize_dataframe_table(ctx, calibration_summary_df, artifact_name(ctx, "table_royston_parmar_tuned_calibration_summary"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_calibration_summary - Royston-Parmar calibration summary")
    materialize_dataframe_table(ctx, calibration_bins_df, artifact_name(ctx, "table_royston_parmar_tuned_calibration_bins_by_horizon"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_calibration_bins_by_horizon - Royston-Parmar calibration bins")
    materialize_dataframe_table(ctx, predicted_vs_observed_survival_df, artifact_name(ctx, "table_royston_parmar_tuned_predicted_vs_observed_survival"), "5.12.3", "Stage 5.12.3 table_royston_parmar_tuned_predicted_vs_observed_survival - Royston-Parmar predicted versus observed survival")

    config_payload = {
        "model_name": MODEL_NAME,
        "selection_metric": "lowest_validation_ibs_among_warning_free_candidates",
        "validation_split": {
        ...
        "preprocessing": {
            ...
            "feature_names_safe": treatment.feature_names_safe,
            "formula_feature_map": treatment.formula_feature_map,
            "dropped_low_variance_features": treatment.dropped_feature_names,
        },
        "design_note": (
            "The comparable Royston-Parmar arm preserves the canonical early-window contract, "
            "uses the same deterministic preprocessing as the other continuous-time baselines, "
            "sanitizes transformed feature names for formula-safe spline fitting, "
            "and rejects candidates that emit fit or prediction warnings under strict execution."
        ),
    }
    save_json(config_payload, config_path)
    print_artifact("royston_parmar_model", str(model_path))
    print_artifact("royston_parmar_preprocessor", str(preprocessor_path))
    print_artifact("royston_parmar_tuned_model_config", str(config_path))
    print(config_path.read_text(encoding="utf-8"))
    print(pd.DataFrame([best_candidate]).to_string(index=False))

    log_stage_end("5.12.3")


def close_context(ctx: PipelineContext) -> None:
    from util import close_duckdb_connection

    log_stage_start("5.12.4", "Close runtime resources")
    close_duckdb_connection(ctx.con)
    print_artifact("duckdb_connection", f"closed::{ctx.duckdb_path}")
    log_stage_end("5.12.4")
...
def run_single_window(window_weeks: int) -> None:
    ...
    try:
        ...
        treatment = build_spline_treatment(ctx)
        tune_and_evaluate_spline_model(ctx, treatment)
    finally:
        if ctx is not None:
            close_context(ctx)
